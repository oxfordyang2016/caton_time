...
*/
/*data structure create table
1.create database hello;
2.use hello;
3.create table who( id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
  sn VARCHAR(255), model VARCHAR(255), version VARCHAR(255), last_login VARCHAR(255));
4.create table report( id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
 sn VARCHAR(255), ip VARCHAR(255),time VARCHAR(255),
  version VARCHAR(255),content_type VARCHAR(255), content_lenth INT(11), msg_body longblob);
5.GRANT ALL ON hello.* TO root@'%' IDENTIFIED BY 'yourRootPassword';
*/
...
func login(rw http.ResponseWriter, req *http.Request) {
	...
	*/
	...
	if err1 != nil {
		rep1 := &Response{Token: "error", Status: 102}
		b1, _ := json.Marshal(rep1)
		fmt.Println(string(b1))
		io.WriteString(rw, string(b1)) //write to body
		return
	}
	fmt.Println(err1)
	...
	if verify_data != password {
		...
		rep1 := &Response{Token: "error", Status: 104}
		b1, _ := json.Marshal(rep1)
		fmt.Println(string(b1))
		//		io.WriteString(rw, string(b1)) //write to body
		//		return
	...
	to, _ := ExampleNew(t.Sn, t.Version, []byte(mySigningKey)) //give jwt token
	fmt.Println(to)
	//first_token = to
	fmt.Println(reflect.TypeOf(to))
	rep := &Response{Token: to, Status: 101}
	...
	*/
	...
	if err_tmp != nil {
		rep1 := &Response{Token: "error", Status: 103}
		b1, _ := json.Marshal(rep1)
		fmt.Println(string(b1))
		io.WriteString(rw, string(b1)) //write to body
		return
...
func report(rw http.ResponseWriter, req *http.Request) {
	...
	*/
	...
	fmt.Println("++++++++++++++++++++++++++++yangmingishere+++++++++++++++++++++++++++++++++++++++++++++++++", authorization)
	//----------second verify---------------
	...
	*/
	...
	report_sn, report_version := ExampleParse(token, "caton")
	...
	//fmt.Println(report_sn1, report_sn, "------------------------------------------------------report_sn_verify___________________________________")
	if report_sn == "error" {
		return
	}
	...
	if err != nil {
		...
		rep1 := &Response{Token: "error", Status: 202}
		b1, _ := json.Marshal(rep1)
		fmt.Println(string(b1))
		io.WriteString(rw, string(b1)) //write to body
		return
	...
	//--------------------report error----------
	rep2 := &Response{Token: "success", Status: 200}
	b2, _ := json.Marshal(rep2)
	fmt.Println(string(b2))
	io.WriteString(rw, string(b2)) //write to body
	return
	//db.Close()
...
func ExampleNew(sn string, version string, mySigningKey []byte) (string, error) {
...
*/
func ExampleParse(myToken string, myKey string) (string, string) {
